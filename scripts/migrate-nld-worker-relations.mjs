...
const RELATIONS = [
  ...
  },
  {
    ...
    oldField: "target_user",
    newField: "target_worker",
    required: false,
    dropOld: false,
  },
  {
...
async function getAll(name, fields) {
  return pb.collection(name).getFullList({ fields });
}
...
const users = await getAll(
  "users",
  "id,role,tenant_company,full_name,username,phone,uid,cccd,cccd_issue_date,gender,date_of_birth,address,bank_name,bank_account_number,bank_account_name,bank_account_note,employee_code,status,company",
);
const legacyUsers = users.filter((u) => u.role === "user" || !u.role);
const workers = await getAll(
  "workers",
  "id,auth_user,tenant_company,full_name,phone,uid,cccd,cccd_issue_date,gender,date_of_birth,address,bank_name,bank_account_number,bank_account_name,bank_account_note,employee_code,status,company,source_user_id",
);
...
for (const user of legacyUsers) {
  ...
  if (!worker && APPLY) {
    const payload = {
      ...
      tenant_company: user.tenant_company || user.company || "",
      company: user.tenant_company || user.company || "",
...
for (const target of RELATIONS) {
  ...
  if (!newField && APPLY) {
    ...
    for (const ruleName of ["listRule", "viewRule", "createRule", "updateRule", "deleteRule"]) {
      ...
      if (typeof rule === "string")
        nextRules[ruleName] = rule.replaceAll(target.oldField, target.newField);
    }
    await pb.collections.update(collection.id, {
      fields: [...collection.fields, relationField(target.newField, workersCollection.id, false)],
      ...nextRules,
    });
  ...
  const records = await getAll(
    target.collection,
    `id,${target.oldField},${target.newField},tenant_company`,
  );
  let migratedCount = 0;
  ...
  for (const record of records) {
    ...
    migratedCount++;
    if (APPLY && newField && record[target.newField] !== workerId)
      await updateRecord(target.collection, record.id, { [target.newField]: workerId });
  }
  report.migrated.push({
    collection: target.collection,
    oldField: target.oldField,
    newField: target.newField,
    migratedCount,
    nonWorkerCount,
    dropOld: target.dropOld,
  });
...
if (DROP_FIELDS && APPLY) {
  ...
  for (const item of RELATIONS.filter((x) => x.dropOld)) {
    ...
    if (
      !old &&
      next &&
      next.collectionId === workersCollection.id &&
      !report.droppedFields.includes(`${item.collection}.${item.oldField}`)
    ) {
...
if (DELETE_USERS && APPLY && report.unresolved.length === 0) {
  ...
  for (const c of remaining) {
    ...
    for (const f of c.fields || []) {
      if (
        f.collectionId === usersCollection.id &&
        ["user", "worker", "target_user"].includes(f.name)
      )
        blockers.push(`${c.name}.${f.name}`);
    }
  }
  if (blockers.length)
    report.unresolved.push({ reason: `Còn relation NLĐ trỏ users: ${blockers.join(", ")}` });
  else {
    for (const user of legacyUsers) {
      try {
        await pb.collection("users").delete(user.id);
        report.deletedUsers.push(user.id);
      } catch (error) {
        report.unresolved.push({
          userId: user.id,
          reason: error?.response?.message || error?.message || String(error),
