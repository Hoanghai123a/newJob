#!/usr/bin/env node
/**
 * Xoá toàn bộ phiếu ứng đã import từ HRP (reason="Import từ HRP")
 *
 * Chạy: node scripts/rollback-advances-hrp.mjs [--apply]
...
if (!APPLY) {
  ...
  if (failures.length > 0) {
    const failFile = path.join(__dirname, "..", "rollback_advances_hrp_errors.json");
