...
const textUpdates = new Map([
  [
    "sh/4bq1wzmp",
    "Một nguồn dữ liệu thống nhất cho Giám đốc, HR và vận hành nhà máy\nRa quyết định nhanh hơn, giảm đối soát, rõ trách nhiệm",
  ],
  ...
  [
    "sh/wzi583ah",
    "Đội ngũ phải dành nhiều giờ để tìm dữ liệu đúng thay vì xử lý công việc chuyên môn.",
  ],
  ...
  [
    "sh/sji1s7q5",
    "Định danh UID, CCCD và tài khoản. Một hồ sơ theo dõi lịch sử đi làm, mã NV, nhà máy và đối tác.",
  ],
  ["sh/qh0jqx8z", "Công và tài chính vận hành"],
  [
    "sh/1oripcrm",
    "Liên kết lịch sử đi làm với công, lương, thâm niên, tạm ứng và giữ lương; mọi yêu cầu đều có trạng thái duyệt.",
  ],
  ["sh/fm90n29w", "Phân quyền và nhật ký"],
  [
    "sh/el0jux8b",
    "Mỗi vai trò chỉ xem phần cần thiết. Lưu ai làm, lúc nào và thay đổi gì để dễ kiểm tra.",
  ],
  ["sh/nqp8ju1k", "Năng lực sẵn sàng cho vận hành"],
  ["sh/7uxsfy9w", "Quản lý tập trung, dễ kiểm soát"],
  [
    "sh/8v6983qh",
    "• Dashboard nhân lực theo thời gian thực: Xử lý 500+ bản ghi mỗi lần tải, lọc theo nhà máy, người tuyển và trạng thái đang làm/đã nghỉ.",
  ],
  [
    "sh/9wfah8rm",
    "• Nhập/xuất Excel theo UID: Dòng lỗi tách riêng, không làm gián đoạn dữ liệu hợp lệ.",
  ],
  [
    "sh/uxobad87",
    "• Kiểm soát rủi ro: Cảnh báo trùng lịch sử đi làm, kiểm tra phiên bản CCCD và bảo mật dữ liệu tài chính.",
  ],
  ...
  [
    "sh/0ni50by1",
    "Không cần hỏi “File Excel nào mới nhất?”. Hệ thống trả lời bốn câu hỏi điều hành:",
  ],
  ...
  [
    "sh/qh0vaxwv",
    "* Khi chạm 80% hạn mức, hệ thống đề xuất nâng gói; quy trình và dữ liệu vẫn giữ nguyên.",
  ],
...
const tableRows = [
  ...
  [
    "Dữ liệu phân tán",
    "Không liền mạch từ tuyển dụng đến tính lương",
    "Một Workspace cho hồ sơ và lịch sử",
  ],
  [
    "Sai tên, trùng mã NV",
    "Khó truy trách nhiệm, rủi ro tài chính",
    "Định danh UID, lưu phiên bản và cảnh báo trùng",
  ],
  ["Báo cáo chậm", "Quyết định dựa trên dữ liệu cũ", "Dashboard cập nhật ngay khi có dữ liệu"],
  [
    "Phê duyệt mơ hồ",
    "Tranh cãi khi có sai sót ứng/giữ lương",
    "Lưu trạng thái, thời điểm và tài khoản duyệt",
  ],
];
for (let r = 0; r < tableRows.length; r += 1)
  for (let c = 0; c < tableRows[r].length; c += 1) table.getCell(r, c).text.set(tableRows[r][c]);
...
for (const slide of presentation.slides.items) {
  ...
  for (const t of slide.tables.items)
    for (let r = 0; r < t.rowCount; r += 1)
      for (let c = 0; c < t.columnCount; c += 1) t.getCell(r, c).text.typeface = "Arial";
}

// Make the ROI comparison transparent about its assumption.
const roiSlide = presentation.slides.items[7];
const note = roiSlide.shapes.add({
  geometry: "textbox",
  position: { left: 60, top: 145, width: 1160, height: 26 },
  fill: "none",
  line: { fill: "none", width: 0 },
});
note.text =
  "Minh họa theo giả định 12 giờ đối soát/tháng; xác nhận lại bằng số liệu thực tế trước khi ký hợp đồng.";
note.text.style = {
  typeface: "Arial",
  fontSize: 14,
  color: "#64748B",
