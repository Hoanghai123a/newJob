...
const skillDir =
  "C:\\Users\\admin\\.codex\\plugins\\cache\\openai-primary-runtime\\presentations\\26.903.11726\\skills\\presentations";
const runtimePython =
...
function addTitle(slide, title, kicker = "") {
  if (kicker)
    addText(slide, kicker.toUpperCase(), 72, 42, 900, 24, {
      fontSize: 14,
      bold: true,
      color: C.teal,
    });
  addText(slide, title, 72, kicker ? 68 : 48, 1100, 62, {
    fontSize: 40,
    bold: true,
    color: C.navy,
  });
...
function addFooter(
  slide,
  page,
  note = "Đề xuất thương mại hóa | Dữ liệu tính năng lấy từ sản phẩm hiện tại",
) {
  addText(slide, note, 72, 682, 930, 18, { fontSize: 11, color: C.slate });
  addText(slide, String(page).padStart(2, "0"), 1160, 678, 48, 24, {
    fontSize: 14,
    bold: true,
    color: C.teal,
    alignment: "right",
...
function addBulletList(
  ...
  items.forEach((item, i) => {
    ...
    addText(slide, item, left + 24, top + i * lineHeight, width - 24, lineHeight, {
      fontSize,
      color,
    });
  });
...
// 1. Cover
{
  ...
  slide.images.add({
    ...
    fit: "contain",
    position: { left: 1000, top: 122, width: 154, height: 154 },
  });
  addText(slide, "Tuyển dụng 4.0", 90, 144, 740, 70, { fontSize: 58, bold: true, color: C.white });
  addText(
    slide,
    "Từ file Excel rời rạc đến một nguồn dữ liệu điều hành thống nhất",
    92,
    244,
    700,
    86,
    { fontSize: 28, color: "#D9F5F3" },
  );
  addBox(slide, 92, 378, 260, 42, C.orange, 21, "none");
  addText(slide, "ĐỀ XUẤT THƯƠNG MẠI HÓA", 112, 388, 220, 22, {
    fontSize: 14,
    bold: true,
    color: C.navy,
    alignment: "center",
  ...
  addText(
    slide,
    "Dành cho giám đốc, quản trị nhân sự và đội ngũ vận hành nhà máy",
    92,
    560,
    740,
    34,
    { fontSize: 18, color: "#B7D1D3" },
  );
  addText(slide, "04.09.2026", 92, 626, 200, 24, { fontSize: 14, color: "#B7D1D3" });
  addNotes(
    slide,
    "Nguồn nội bộ: nhận diện ứng dụng từ public/icons/app-icon-512.png. Nội dung tính năng đối chiếu PROJECT_MAP.md và mã nguồn hiện tại.",
  );
}
...
// 2. Excel pain
{
  ...
  addText(
    slide,
    "Khi mỗi bộ phận giữ một file, người ra quyết định phải ghép dữ liệu bằng tay trước khi hiểu chuyện gì đang xảy ra.",
    72,
    136,
    850,
    52,
  ...
  addText(slide, "5 hệ quả thường gặp", 104, 255, 420, 32, {
    fontSize: 24,
    bold: true,
    color: C.navy,
  });
  ...
  addText(slide, "Một vòng lặp thủ công", 700, 255, 440, 32, {
    fontSize: 24,
    bold: true,
    color: C.white,
  });
  ...
  steps.forEach((s, i) => {
    ...
    addText(slide, s, x + 10, y + 11, 180, 20, {
      fontSize: 15,
      bold: true,
  ...
  addText(
    slide,
    "Mất thời gian không chỉ ở nhập liệu, mà ở việc xác định đâu là sự thật.",
    700,
    500,
    430,
    46,
    { fontSize: 19, color: "#D9F5F3" },
  );
  addFooter(slide, 2);
  addNotes(
    slide,
    "Các hệ quả là tổng hợp vấn đề vận hành được mô tả trong yêu cầu thương mại hóa. Đây là framing bán hàng, không phải số liệu khảo sát thị trường.",
  );
}
...
// 3. Single source of truth
{
  ...
  addText(
    slide,
    "Ứng dụng nối liền vòng đời người lao động từ lúc tuyển đến khi tính công, ứng lương và lưu vết xử lý.",
    72,
    136,
    900,
    45,
  ...
  nodes.forEach(([x, y, w, h, title, sub, tone], i) => {
    ...
    if (i < nodes.length - 1)
      addText(slide, "→", x + w + 7, y + 48, 28, 26, {
        fontSize: 23,
        bold: true,
        color: C.teal,
        alignment: "center",
  ...
  addText(
    slide,
    "Khi một dữ liệu thay đổi, các vai trò liên quan nhìn thấy cùng một trạng thái đã được phân quyền.",
    108,
    501,
    1050,
    38,
  ...
  addNotes(
    slide,
    "Nguồn nội bộ: src/lib/employment.ts, src/lib/recruiters.ts, src/lib/staff-log.ts, src/lib/salary.ts và PROJECT_MAP.md.",
  );
}
...
// 4. Product evidence
{
  ...
  const featureRows = [
    ...
    [
      "Nhập và xuất Excel",
      "Nhập mới/cập nhật theo UID; dòng lỗi được tách để sửa, không làm dừng dòng hợp lệ.",
    ],
    ["Tài chính vận hành", "Bảng công, bảng lương, ứng lương, giữ lương và quy trình phê duyệt."],
    [
      "Kiểm soát dữ liệu",
      "Chặn lịch sử đang làm trùng, theo dõi CCCD phiên bản và lưu nhật ký thao tác.",
    ],
  ];
  ...
  addNotes(
    slide,
    "Nguồn nội bộ: PROJECT_MAP.md; src/routes/_authenticated/admin/workforce.tsx; src/routes/_authenticated/admin/imports.tsx; src/lib/salary.ts; src/lib/uid-counter.ts; src/components/admin/AccountActivityStats.tsx.",
  );
}
...
// 5. Pain to fix table
{
  ...
  const table = slide.tables.add({
    ...
    values: [
      ...
      [
        "Dữ liệu phân tán",
        "Không xuyên suốt giữa tuyển dụng, nhà máy và lương",
        "Gom hồ sơ, lịch sử và quan hệ vào một tenant công ty",
      ],
      [
        "Sai tên và mã",
        "Trùng hoặc nhầm người, khó truy ngược",
        "UID, snapshot tại nhà máy và kiểm tra trùng lịch sử",
      ],
      [
        "Báo cáo chậm",
        "Giám đốc ra quyết định sau khi sự việc đã xảy ra",
        "Dashboard lọc theo kỳ, nhà máy và người tuyển",
      ],
      [
        "Phê duyệt mơ hồ",
        "Không rõ ai chịu trách nhiệm",
        "Trạng thái phê duyệt và nhật ký actor / thời điểm",
      ],
      [
        "Đối soát thủ công",
        "Tốn giờ tổng hợp, dễ bỏ sót",
        "Nhập Excel có template, báo lỗi theo từng dòng, xuất lại file lỗi",
      ],
    ],
  });
  ...
  addText(
    slide,
    "Mục tiêu thương mại hóa: biến dữ liệu vận hành thành tài sản dùng chung, không phải file của riêng một người.",
    72,
    603,
    1136,
    38,
  ...
  addNotes(
    slide,
    "Bảng tổng hợp lợi ích định tính từ các luồng tính năng hiện có. Không gắn số liệu năng suất chưa được đo tại khách hàng.",
  );
}
...
// 6. Executive view
{
  ...
  addText(
    slide,
    "Một màn hình tốt phải trả lời được câu hỏi tiếp theo, không chỉ hiển thị thêm dữ liệu.",
    72,
    136,
    850,
    40,
  ...
  addText(slide, "Câu hỏi điều hành", 108, 244, 410, 32, {
    fontSize: 24,
    bold: true,
    color: C.white,
  });
  ...
  addText(slide, "Câu trả lời từ hệ thống", 656, 244, 490, 32, {
    fontSize: 24,
    bold: true,
    color: C.navy,
  });
  ...
  addText(
    slide,
    "Kết quả: cuộc họp bắt đầu từ quyết định, không bắt đầu từ việc hỏi “file nào mới nhất?”.",
    72,
    612,
    1136,
    30,
  ...
  addNotes(
    slide,
    "Nguồn nội bộ: dashboard nhân lực và các component workforce / approval / hour-stats trong PROJECT_MAP.md.",
  );
}
...
// 7. Pricing table
{
  ...
  addText(
    slide,
    "Đơn vị tính theo dữ liệu cần quản lý. Mỗi gói gồm một công ty, hỗ trợ onboarding từ xa và cập nhật tính năng nền tảng.",
    72,
    136,
    1050,
    40,
  ...
  addText(
    slide,
    "Nguyên tắc mở rộng: nâng gói khi chạm 80% hạn mức; không cần thay đổi quy trình hay di chuyển dữ liệu.",
    94,
    575,
    1090,
    26,
    { fontSize: 18, bold: true, color: "#805A00", alignment: "center" },
  );
  addFooter(
    slide,
    7,
    "Đề xuất giá tham khảo | Có thể điều chỉnh theo số công ty, SLA và nhu cầu tích hợp",
  );
  addNotes(
    slide,
    "Đây là đề xuất giá tham khảo theo yêu cầu người dùng, không phải bảng giá đã được phê duyệt. Hạn mức minh họa: lịch sử đi làm, tài khoản staff, đối tác và nhà máy.",
  );
}
...
// 8. ROI chart
{
  ...
  addText(
    slide,
    "Mô hình minh họa: chỉ cần giảm một phần thời gian đối soát mỗi tháng, gói phần mềm đã có thể tự hoàn vốn.",
    72,
    136,
    1080,
    40,
  ...
  const chart = slide.charts.add("bar", {
    ...
    yAxis: {
      ...
      title: { text: "Triệu đồng / tháng" },
      majorGridlines: { style: "solid", fill: C.line, width: 1 },
      textStyle: { typeface: family, fontSize: 13, fill: C.slate },
    },
    xAxis: { textStyle: { typeface: family, fontSize: 13, fill: C.slate } },
    dataLabels: {
      showValue: true,
      position: "outEnd",
      textStyle: { typeface: family, fontSize: 12, fill: C.ink, bold: true },
    },
  });
  applyPresentationChartFont(chart, { fontFamily: family });
  addBox(slide, 900, 218, 308, 350, C.white, 18, C.line);
  addText(slide, "Giả định minh họa", 932, 250, 245, 28, {
    fontSize: 22,
    bold: true,
    color: C.navy,
  });
  ...
  addText(slide, "Khởi động: 2,4×", 944, 534, 220, 16, {
    fontSize: 15,
    bold: true,
    color: C.green,
    alignment: "center",
  ...
  addNotes(
    slide,
    "Biểu đồ không phải số liệu khách hàng. Giả định minh họa: 10 giờ đối soát giảm mỗi tháng x 120.000 đ/giờ = 1,2 triệu đ; các gói lớn dùng giả định thời gian tiết kiệm cao hơn để minh họa logic định giá.",
  );
}
...
// 9. Pilot roadmap
{
  ...
  addText(
    slide,
    "Bắt đầu nhỏ, đo được, và chỉ mở rộng khi người dùng đã nhìn thấy dữ liệu của chính họ.",
    72,
    136,
    920,
    40,
  ...
  const weeks = [
    ...
    [
      "Tuần 2",
      "Di chuyển dữ liệu",
      "Nhập Excel theo template; xử lý các dòng lỗi còn lại.",
      C.cyan,
    ],
    [
      "Tuần 3",
      "Đưa vào nhịp",
      "Chạy luồng hồ sơ, lịch sử đi làm, phê duyệt và công/lương.",
      C.orange,
    ],
    [
      "Tuần 4",
      "Đo và mở rộng",
      "Đo giờ đối soát, lỗi dữ liệu, thời gian ra báo cáo; chọn gói phù hợp.",
      C.green,
    ],
  ];
  ...
  addText(
    slide,
    "Đầu ra sau pilot: số liệu trước/sau, danh sách lỗi đã giảm và đề xuất gói theo quy mô thật.",
    100,
    568,
    1080,
    28,
    { fontSize: 21, bold: true, color: C.white, alignment: "center" },
  );
  addFooter(slide, 9);
  addNotes(
    slide,
    "Lộ trình là đề xuất triển khai thương mại, không phải tính năng đã có sẵn trong ứng dụng.",
  );
}
...
// 10. Close
{
  ...
  addText(slide, "Bắt đầu với gói Khởi động", 92, 142, 850, 64, {
    fontSize: 48,
    bold: true,
    color: C.white,
  });
  addText(slide, "500.000 đ / tháng", 94, 238, 500, 52, {
    fontSize: 34,
    bold: true,
    color: C.orange,
  });
  addText(slide, "500 lịch sử đi làm · 5 staff · 5 đối tác · 5 nhà máy", 94, 312, 760, 34, {
    fontSize: 22,
    color: "#D9F5F3",
  });
  ...
  addText(
    slide,
    "Mục tiêu đầu tiên: một nguồn dữ liệu mà cả công ty cùng tin.",
    122,
    430,
    485,
    32,
  ...
  addText(
    slide,
    "Đề xuất tiếp theo: chọn một công ty hoặc một cụm nhà máy để chạy pilot 30 ngày.",
    94,
    594,
    850,
    30,
    { fontSize: 19, color: "#B7D1D3" },
  );
  addText(slide, "Tuyển dụng 4.0", 1012, 622, 170, 24, {
    fontSize: 16,
    bold: true,
    color: C.white,
    alignment: "right",
  ...
  addNotes(
    slide,
    "Đề xuất CTA cho buổi bán hàng. Giá và pilot cần được phê duyệt trước khi công bố chính thức.",
  );
}
...
await fs.writeFile(
  path.join(buildDir, "montage.webp"),
